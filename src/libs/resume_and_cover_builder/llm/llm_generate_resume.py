...
class LLMResumer:
    def __init__(self, openai_api_key, strings):
        ...
        logger.info(f"LLMResumer初始化，使用模型类型: {global_config.MODEL_TYPE}，模型: {global_config.MODEL}")
        
        if global_config.MODEL_TYPE.lower() == "ollama":
            # 使用完整的模型名称，不再尝试分离冒号
            model_name = global_config.MODEL
            logger.info(f"使用Ollama模型: {model_name}")
            llm = ChatOllama(
                model=model_name,
                base_url=global_config.LLM_API_URL,
                temperature=0.7
            )
        else:
            model_name = global_config.MODEL or "gpt-4o-mini"
        ...
        self.llm_cheap = LoggerChatModel(llm=llm)
